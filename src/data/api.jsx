...
export const getInitialData = () => {
  ...
  return Promise.all([
    ...
    axios.get(import.meta.env.VITE_BLOCKLIST_WALLET), // loads blocked wallets
    axios.get(import.meta.env.VITE_BLOCKLIST_BAN), // blocked wallets (dont allow to visualise in /tz/walletid)
  ]).then((results) => {
...
export const GethDAOFeed = async ({ counter }) => {
  return new Promise((resolve, reject) => {
    axios
      .get(import.meta.env.VITE_FEED_HDAO, {
...
export const GetFeaturedFeed = async ({ counter, max_time }) => {
  return new Promise((resolve, reject) => {
    axios
      .get(import.meta.env.VITE_FEATURED, {
        params: { counter: counter, max_time: max_time },
      })
      .then((res) => {
        resolve(filterFeeds(res.data.result))
...
export const GetRandomFeed = async ({ counter }) => {
  return new Promise((resolve, reject) => {
    axios
      .get(import.meta.env.VITE_RANDOM, {
        params: { counter: counter },
      })
      .then((res) => {
        resolve(filterFeeds(res.data.result))
...
export const GetOBJKT = async ({ id }) => {
  return new Promise((resolve, reject) => {
    axios
      .get(import.meta.env.VITE_OBJKT, {
        params: { id: id },
      })
      .then((res) => {
...
export const GetTags = async ({ tag, counter }) => {
  return new Promise((resolve, reject) => {
    axios
      .post(import.meta.env.VITE_TAGS, { tag: tag, counter: counter })
      .then((res) => {
        // console.log(res.data)
        resolve(res.data.result)
      })
