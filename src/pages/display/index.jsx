...
export default class Display extends Component {
  ...
  componentDidMount = async () => {
    ...
    if (id === 'tz') {
      ...
      if (res.length >= 1) {
        ...
        let resTz = await fetchTz(res[0].address)
        this.setState({ hdao: Math.floor(resTz[0].hdao_balance / 1000000) })
      } else {
        this.props.history.push('/')
      }

      await GetUserMetadata(res[0].address).then((data) => {
        const {
          dns,
          github,
          discord,
  ...
  creations = async () => {
    ...
    if (!list.includes(this.state.wallet)) {
      creations = await fetchCreations(this.state.wallet) 
      this.setState({ creations: creations })
      this.setState({ objkts: creations, loading: false, items: [] })
      this.setState({ marketV1: await fetchV1Swaps(this.state.wallet) })
    } else {
      this.setState({ restricted: true, loading: false })
    }

    this.setState({ items: creations.slice(0, 15), offset: 15 })
  ...
  collectionFull = async () => {
    ...
    if (!list.includes(this.state.wallet)) {
      ...
      collection = await fetchCollection(this.state.wallet)
      let offset = 0
      ...
      let v1 =  await fetchV1Swaps(this.state.wallet)
      this.sortCollection(combinedCollection)
      this.setState({ collection: combinedCollection })
      this.setState({ marketV1: v1 })
    } else {
      this.setState({ restricted: true, loading: false })
    }

    this.setState({ objkts: collection, loading: false, items: [] })
    this.setState({ items: collection.slice(0, 15), offset: 15 })
