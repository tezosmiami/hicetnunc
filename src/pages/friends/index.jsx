...
export class Friends extends Component {
  ...
  loadMore = async () => {
    let result = await this.fetchFeed(this.state.frens)
    this.setState({
  ...
  componentDidMount = async () => {
    !this.props.feedstyle && this.setState({
  ...
  onReady = async () => {
    ...
    const getLatestByFrens = async () => {
      try {
        ...
        return this.fetchFeed(frensAddresses)
      } catch (error) {
        console.log(error)
      }
    }
    ...
    return (
      <Page title={this.state.alias} feed={this.props.wallet ? true : false} >
           ...
           {!this.props.wallet && 
            <Container>
              <Padding>
                <div style={{display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', paddingBottom: this.state.feedstyle=== 'original' ? '11px' : '', marginRight: '18px'}}>
