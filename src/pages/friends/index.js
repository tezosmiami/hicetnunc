...
export class Friends extends Component {
  ...
  state = {
    ...
    render: false,
    loading: true,
    feedstyle: getItem('feedstyle', 'post') ? 'post' : 'original',
    frens: [],
    creations: [],
  }
    ...
    return (
      <Page title={this.state.alias} feed={this.props.wallet ? true : false} >
           {!this.props.wallet && <div style={{display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', marginRight: '18px'}}>
            <Button onClick={this.state.switchStyle}>
              <Primary>
              ...
              <Container>
                <Padding>
                  {this.state.creations.map((item, index) => (
                    this.props.feedstyle === 'original' ? <FeedItem key={`${item.id}-${index}`} {...item} creator_id={item.creator.address} /> 
